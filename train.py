...
def train_epoch_multimodal(epoch, data_loader, model, criterion, optimizer, opt,
    ...
    for i, v in enumerate(data_loader):
        audio_inputs, visual_inputs, targets = v
        #print('train28>>>>>>>>>>>>>>>>>>>>>>>>>audioinput',audio_inputs.shape)
        ...
        if opt.mask is not None:
            with torch.no_grad():
                ...
                if opt.mask == 'noise':
                    ...
                    coefficients = coefficients.repeat(1,audio_inputs.size(1),audio_inputs.size(1))
        ...
        audio_inputs = audio_inputs.to(dtype=torch.float)
        audio_inputs = Variable(audio_inputs)
        visual_inputs = Variable(visual_inputs).to(dtype=torch.float)
        #print('train70>>>>>>>>>>>>>>>>>>>>>>>>>audioinput',audio_inputs.dtype)
        #print('train71>>>>>>>>>>>>>>>>>>>>>>>>>visual_inputs',visual_inputs.dtype)
...
def train_epoch(epoch, data_loader, model, criterion, optimizer, opt,
                epoch_logger, batch_logger):
    #print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPP",data_loader.dataset.data)
